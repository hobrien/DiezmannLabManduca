...
## 1c - Weight gain at 25˚C  and 37˚C
...
(weight_25_versus_37C_plot <- weight_25_versus_37C %>%  ggplot(aes(x=as.numeric(str_remove(Day, 'Day')), y=weight_change, colour=Temperature)) +
  ...
  #scale_x_continuous(limits=c(1,4), expand=expand_scale(mult = 0, add = 0)) +
  ylab("% Weight Change") +
  xlab("") +
...
(heat_killed_weight_plot <- heat_killed_weight %>% 
  ...
  geom_line( alpha=.2) +
  scale_x_continuous(limits=c(1,2), expand=expand_scale(mult = 0, add = 0)) +
  ylab("% Weight Change") +
  xlab("") +
  guides(colour = guide_legend(override.aes = list(alpha = 1))) +
...
DE[is.na(DE)]<-0
pdf(ffFigFolder("chart2.pdf"), width=6, height=4)
