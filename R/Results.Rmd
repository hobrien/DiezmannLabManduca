...
drc_weight_37C <- drc_survival_weight_37C %>% filter(Category == 'Weight') %>%
  ...
  mutate(Animal=paste(Pot, Treatment, Replicate, sep='-'),
         Treatment=factor(Treatment),
         weight_change=(Weight-starting_weight)/starting_weight*100,
         Day=as.numeric(str_remove(Day, 'Day'))) 
(drc_weight_37C_plot <- drc_weight_37C %>% 
  ggplot(aes(x=Day, y=weight_change, colour=Treatment, group=Animal)) +
    geom_line(size=.25) +
    geom_point(size=.5) +
...
fluconazole_weight <- fluconazole_weight %>%
  mutate(starting_weight=Day1) %>%
  gather('Day', 'Weight', starts_with('Day')) %>%
  mutate(Treatment=factor(Treatment, levels=c('0 mg/kg' , '1 mg/kg' , '4 mg/kg',
...
```{r Heat_killed_37C}
heat_killed_weight <- read_tsv(ffDataFolder("heat_killed_weight.txt")) %>%
    rownames_to_column() %>%
    mutate(starting_weight=Day1) %>%
    gather('Day', 'Weight', starts_with('Day')) %>% 
