...
plot_pair <- function(conc) {
  data_t <- fluconazole_survival %>% 
    filter(Treatment %in% c('0 mg/kg', conc)) %>%
    convert_srv_data()
  make_surv_plot(data_t, "")
}
lapply(concentrations, plot_pair)
```
...
- growth is slightly slower at 37 deg (p=0.022)
...
weight_25_versus_37C <- weight_25_versus_37C %>% 
  ...
  mutate(Animal=paste0(Temperature, Experimentalist, Animal),
         ...
         weight_change=(Weight-starting_weight)/starting_weight*100,
         Day=as.numeric(str_remove(Day, 'Day')))
lm_fit <- lme(weight_change ~ Day + Day:Temperature, random=~1|Animal, data=weight_25_versus_37C, method='ML')
lm_fit2 <- lme(weight_change ~ Day, random=~1|Animal, data=weight_25_versus_37C, method='ML')
anova(lm_fit, lm_fit2)
(weight_25_versus_37C_plot <- weight_25_versus_37C %>%  ggplot(aes(x=as.numeric(str_remove(Day, 'Day')), y=weight_change, colour=Temperature)) +
  geom_line(aes(group=Animal), alpha=1) +
  ylab("% Weight Change") +
  xlab("Day") +
