...
mod_37 <- lme(Weight ~ Day + Treatment:Day, random= ~1|rowname, data=filter(drc_weight_37C,  Day < 4 & !is.na(Weight)))
summary(mod_37)
```
...
## 3c - Fungal burden and caterpillar weight
...
ggsave(ffPanelFolder("Fig.3_fungal_burden.pdf"), plot=combined_plot, height=8, units="in")
```


# Fig 4 - Treatment with antifungal drugs cures caterpillars from fungal disease

## 4a - Fluconazole survival
...
## 4b - Fluconazole treatment and weight
...
mod_flu <- lme(Weight ~ Day + Treatment:Day, random= ~1|Pot, data=filter(fluconazole_weight, Day < 4 & !is.na(Weight) & Treatment != 'PBS'), method='ML')
summary(mod_flu)

mod_flu_null <- lme(Weight ~ Day, random= ~1|Pot, data=filter(fluconazole_weight, Day < 4 & !is.na(Weight) & Treatment != 'PBS'), method='ML')
anova(mod_flu, mod_flu_null)
```

## 4c - Caspofungin survival
...
```
## 4d - Caspofungin treatment and weight
...
mod_cas <- lme(Weight ~ Day + Treatment:Day, random= ~1|Pot, data=filter(caspofungin_weight, Day < 4 & !is.na(Weight) & Treatment != 'PBS'), method='ML')
summary(mod_cas)

mod_cas_null <- lme(Weight ~ Day, random= ~1|Pot, data=filter(caspofungin_weight, Day < 4 & !is.na(Weight) & Treatment != 'PBS'), method='ML')
...
mod_ca <- lme(Weight ~ Day + Treatment:Day, random= ~1|Animal, data=filter(drc_weight_species, Species=='C. albicans' & Day<3 & !is.na(Weight) & Treatment != '10^8'))
summary(mod_ca)


# C. auris grows marginally slower at lower concentrations and significantly slower at 10^8 and 10^9
mod_cau <- lme(Weight ~ Day + Treatment:Day, random= ~1|Animal, data=filter(drc_weight_species, Species=='C. auris' & Day<3 & !is.na(Weight)))
summary(mod_cau)
# C. glabrata grows marginally slower at lower concentrations and significantly slower at 10^7, 10^8 and 10^9
mod_cg <- lme(Weight ~ Day + Treatment:Day, random= ~1|Animal, data=filter(drc_weight_species, Species=='C. glabrata' & Day<3 & !is.na(Weight)))
summary(mod_cg)
...
mod_cn <- lme(Weight ~ Day + Treatment:Day, random= ~1|Animal, data=filter(drc_weight_species, Species=='C. neoformans' & Day<3 & !is.na(Weight)))
summary(mod_cn)
...
mod_mp <- lme(Weight ~ Day + Treatment:Day, random= ~1|Animal, data=filter(drc_weight_species, Species=='M. pulcherrima' & Day<3 & !is.na(Weight)))
summary(mod_mp)
```
